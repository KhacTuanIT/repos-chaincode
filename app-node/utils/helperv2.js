...
const enrollAdmin = async (org) => {
    ...
    try {
        ...
        throw new Error({ message: `Failed to enroll admin user "admin": ${error}` });
    }
}
...
const getProduct = async function (org, productCode) {
    try {
        ...
        const result = await contract.evaluateTransaction('queryProduct', productCode);
        return result;
    } catch (error) {
...
const addProduct = async function (product, org) {
    try {
        ...
        console.log('pass 2');
        const result = await contract.submitTransaction('createProductLedger',
            product.name, product.code, product.manufactororId, product.color, product.price, product.cpu, product.ram,
            product.screen, product.keyboard, product.storage, product.network, product.usb, product.origin,
            product.yearOrigin, product.owner, product.primaryImage, product.subImage
        );
        return result;
    } catch (error) {
        throw new Error(error);
