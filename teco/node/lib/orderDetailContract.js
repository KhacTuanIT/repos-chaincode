...
class OrderDetail extends Contract {

    async initOrderDetail(ctx) {
        return;
    }

    async createOrderDetail(ctx, orderDetailId, orderId, productId, price, quantity) {
        const orderDetail = {
        ...
        try {
            await ctx.stub.putState(orderDetailId, Buffer.from(JSON.stringify(orderDetail)));
    ...
    async updateOrderDetail(ctx, orderDetailId, orderId, productId, price, quantity) {
        const orderDetailAsBytes = await ctx.stub.getState(orderDetailId);
        if (!orderDetailAsBytes || orderDetailAsBytes.length === 0) {
            throw new Error(`OrderDetail ${orderDetailId} does not exist`);
        }
